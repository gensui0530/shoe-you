...
debug('====================================================');
debug('パスワード発行メール送信ページ');
debug('====================================================');
debugLogStart();

//ログイン認証話（ログインできない人が使うから）
...
if (!empty($_POST)) {
    ...
    if (empty($err_msg)) {
        ...
        if (empty($err_msg)) {
         ...
         パスワード再発行認証キー入力ページ：http://localhost:8888/shoe_you/passRemindRecieve.php
         認証キー：{$auth_key}
         ※認証キーの有効期限は30分となります
         
         認証キーを再発行されたい場合は下記ページより再度再発行をお願い致します。
         http://localhost:8888/shoe_you/passRemindSend.php
         
         ////////////////////////////////////////
         Shoe you カスタマーセンター
         URL  http://shoe_you.com/
         E-mail info@shoe_you.com
         ////////////////////////////////////////
         EOT;
                    ...
                    $_SESSION['auth_key_limit'] = time() + (60 * 30); //現在時刻により30分後のUNIXタイムスタンプを入れる
                    debug('セッション変数の中身：' . print_r($_SESSION, true));

                    header("Location:passRemindReceive.php"); //認証キー入力ページへ

                } else {
                    debug('クエリに失敗したがDBに登録のないEmailが入力されました');
